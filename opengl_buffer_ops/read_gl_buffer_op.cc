...
class ReadGlBufferOp : public OpKernel {
 public:
  explicit ReadGlBufferOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
    ...
    // unsigned char *test_display = (unsigned char*) ((long int) egl_display_int_ptr);
    // printf("XXX Op EGL Display Pointer %ld %c\n", (long int) egl_display_int_ptr, *test_display);
